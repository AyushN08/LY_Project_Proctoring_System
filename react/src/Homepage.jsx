...
const Homepage = () => {
  ...
  const startTest = async () => {
    try {
      // Step 1: Ask for webcam + mic access
      await navigator.mediaDevices.getUserMedia({ video: true, audio: true });
  
      // Step 2: Call backend to start proctoring agent
      const res = await fetch("http://localhost:8000/api/start-proctoring", { method: "POST" });
      const data = await res.json();
  
      if (data.status === "Proctoring started" || data.status === "Already running") {
        setTestStarted(true);
        setError("");
      } else {
        setError("Failed to start proctoring. Try again.");
      }
    } catch (err) {
